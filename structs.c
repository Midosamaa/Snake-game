...
Arena* get_arena(int* sizeX, int* sizeY, int* nbWalls, int *walls) {
    ...
    for (int i = 0; i < 4 * (*nbWalls); i += 4) {
        if (walls[i] < *sizeX && walls[i+1] < *sizeY) {
...
void print_arena(Arena* arena) {
    ...
    for (int i = 0; i < arena->sizex-1; i++) {
        ...
        for (int j = 0; j < arena->sizey; j++) {
            ...
            printf("[%d, %d, %d, %d]\n", arena->grid[i][j].borders[0], arena->grid[i][j].borders[1], arena->grid[i][j].borders[2], arena->grid[i][j].borders[3]);
        }
    }
}
